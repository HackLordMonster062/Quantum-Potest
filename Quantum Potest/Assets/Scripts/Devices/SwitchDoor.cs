...
public class SwitchDoor : Activatable, ISerializableElement {
	...
	public ElementData Serialize() {
		return new ActivatableData("SignalDoor", transform.localPosition, transform.eulerAngles, gameObject.GetEntityId().ToString(), _trigger == null ? "" : _trigger.gameObject.GetEntityId().ToString());
	}
