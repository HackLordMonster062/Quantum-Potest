...
public class RotatingDevice : Activatable, ISerializableElement {
	...
	public ElementData Serialize() {
		return new ActivatableData("Rotator", transform.localPosition, transform.eulerAngles, transform.localScale, gameObject.GetEntityId().ToString(), _trigger == null ? "" : _trigger.gameObject.GetEntityId().ToString());
	}
